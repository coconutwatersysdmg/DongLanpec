...
def twoDgeneration(product_id):
    acad, doc = open_drawing_with_wait(r'BES投标图-2.dwg')
    if not doc:
        print("❌ 图纸未打开成功，流程中止。")
        return    # 提取文字类对象
    ...
    modify_text_by_handle(doc, "80D4B", koukou_size_map.get("N5", ""))
    modify_text_by_handle(doc, "80D53", koukou_size_map.get("N6", ""))
    ...
    for handle, code in zip(["778B0", "778C9", "778CA", "778CB","80D5B","80D5C"], ["N1", "N2", "N3", "N4","N5","N6"]):
    ...
    modify_text_by_handle(doc,"77816","管箱封头")
