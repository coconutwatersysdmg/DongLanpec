...
class TubeLayoutEditor(QMainWindow):
    ...
    def update_baffle_parameters_1(self, changed_param_name):
        """
        固定"折流板外径"，当壳体内直径变化时，更新折流板切口与中心线间距和折流板要求切口率(%)
        :param changed_param_name: 发生变化的参数名称
        """
        # 查找四个关键参数在表格中的行索引和当前值
        shell_inner_diameter_row = None  # 壳体内直径 Di
        baffle_diameter_row = None  # 折流板外径（固定值）
        cut_spacing_row = None  # 折流板切口与中心线间距（待更新）
        cut_rate_row = None  # 折流板要求切口率 (%)（待更新）
        ...
        for row in range(self.param_table.rowCount()):
            ...
            if param_name == "壳体内直径 Di":
                ...
                try:
                    baffle_diameter = float(param_value)
                    # 验证折流板外径合理性（必须小于壳体内直径，保留安装间隙）
                    if shell_inner_diameter is not None and baffle_diameter >= shell_inner_diameter:
                        QMessageBox.warning(self, "参数异常",
                                            f"折流板外径({baffle_diameter:.1f}mm)不能大于等于壳体内直径({shell_inner_diameter:.1f}mm)！")
                        baffle_diameter = None
                except ValueError:
                    baffle_diameter = None
            elif param_name == "折流板切口与中心线间距":
        ...
        # 检查必要参数是否存在且有效（折流板外径必须固定且有效）
        if not all([shell_inner_diameter_row is not None,
                    baffle_diameter_row is not None,
                    cut_spacing_row is not None,
                    cut_rate_row is not None,
                    shell_inner_diameter is not None,
                    baffle_diameter is not None]):
            return  # 必要参数缺失或无效，不执行更新

        # 禁用事件触发，避免循环更新（防止更新间距/切口率时再次触发本函数）
        self._is_validating = True

        try:
            # 核心逻辑：固定折流板外径，计算并更新切口间距和切口率
            baffle_radius = baffle_diameter / 2  # 折流板半径（由固定外径推导）

            # 1. 计算合理的切口尺寸（默认取壳体内直径的20%，可根据实际场景调整比例）
            # 说明：此处比例可自定义，示例取20%是工业中常见的折流板切口率参考值
            default_cut_ratio = 0.2  # 20%
            cut_size = shell_inner_diameter * default_cut_ratio

            # 2. 计算折流板切口与中心线间距（B = 折流板半径 - 切口尺寸）
            new_cut_spacing = baffle_radius - cut_size

            # 3. 反算折流板要求切口率（A = (折流板半径 - 切口间距) / 壳体内直径 × 100%）
            new_cut_rate = ((baffle_radius - new_cut_spacing) / shell_inner_diameter) * 100

            # 双重验证：确保计算结果在合理范围内
            # 验证1：切口间距必须在 [0, 折流板半径] 之间（不能为负，也不能超过折流板半径）
            if not (0 <= new_cut_spacing <= baffle_radius):
                QMessageBox.warning(self, "计算错误",
                                    f"切口间距({new_cut_spacing:.1f}mm)超出合理范围(0-{baffle_radius:.1f}mm)，请检查壳体内直径或折流板外径！")
                return
            # 验证2：切口率必须在 [0, 50]% 之间（工业标准中折流板切口率通常不超过50%）
            if not (0 <= new_cut_rate <= 50):
                QMessageBox.warning(self, "计算错误",
                                    f"切口率({new_cut_rate:.1f}%)超出合理范围(0-50%)，请调整切口尺寸比例！")
                return

            # 4. 更新两个目标参数（保留1位小数，符合工程精度要求）
            # 更新折流板切口与中心线间距
            spacing_item = self.param_table.item(cut_spacing_row, 2)
            if spacing_item:
                spacing_item.setText(f"{new_cut_spacing:.1f}")
            # 更新折流板要求切口率(%)
            rate_item = self.param_table.item(cut_rate_row, 2)
            if rate_item:
                rate_item.setText(f"{new_cut_rate:.1f}")
            ...
            logging.error(f"固定折流板外径更新参数失败: {str(e)}")
            QMessageBox.warning(self, "计算错误", f"参数计算过程中发生错误: {str(e)}")
        finally:
            # 恢复事件触发，允许后续正常的参数更新逻辑
            self._is_validating = False

    def update_baffle_parameters_2(self, changed_param_name):
        """
        固定"折流板切口与中心线间距"，当壳体内直径变化时，更新折流板外径和折流板要求切口率(%)
        :param changed_param_name: 发生变化的参数名称
        """
        # 查找四个关键参数在表格中的行索引和当前值
        shell_inner_diameter_row = None  # 壳体内直径 Di
        ...
        try:
            # 固定切口与中心线间距，当壳体内直径变化时，更新折流板外径和切口率
            if cut_spacing is not None and shell_inner_diameter is not None and shell_inner_diameter > 0:
                # 折流板外径通常略小于壳体内直径（保留1mm间隙，可根据实际需求调整）
                new_baffle_diameter = shell_inner_diameter - 1
                baffle_radius = new_baffle_diameter / 2
                ...
                # 更新折流板外径，保留1位小数
                baffle_item = self.param_table.item(baffle_diameter_row, 2)
                if baffle_item:
                    baffle_item.setText(f"{new_baffle_diameter:.1f}")
    ...
    def update_baffle_parameters_3(self, changed_param_name):
        """
        当壳体内直径变化时，固定折流板要求切口率(%)，更新折流板外径和折流板切口与中心线间距
        :param changed_param_name: 发生变化的参数名称
        """
        ...
        try:
            # 固定切口率，当壳体内直径变化时，更新折流板外径和切口间距
            if cut_rate is not None and shell_inner_diameter is not None:
                # 验证切口率范围 [0,50]%
                if not (0 <= cut_rate <= 50):
                ...
                # 计算切口尺寸和新的间距
                cut_size = (cut_rate / 100) * shell_inner_diameter  # 基于固定切口率计算
                new_spacing = baffle_radius - cut_size

                # 验证计算结果有效性
                if new_baffle_diameter <= 0:
                    QMessageBox.warning(self, "计算错误", "折流板外径不能为负数或零！")
                    return
    ...
    def on_cross_pipes_click(self):
        ...
        self.print_cross_x_up_line2 = [
            (i + 1, point[0], point[1]) for i, point in enumerate(self.print_cross_x_up_line2)
        ]
        self.print_cross_x_down_line2 = [
            (i + 1, point[0], point[1]) for i, point in enumerate(self.print_cross_x_down_line2)
        ]
        self.print_cross_x_up_line3 = [
            (i + 1, point[0], point[1]) for i, point in enumerate(self.print_cross_x_up_line3)
        ]
        self.print_cross_x_down_line3 = [
            (i + 1, point[0], point[1]) for i, point in enumerate(self.print_cross_x_down_line3)
        ]
        self.print_cross_y_left_line1 = [
        ...
        self.print_cross_y_left_line2 = [
            (i + 1, point[0], point[1]) for i, point in enumerate(self.print_cross_y_left_line2)
        ]
        self.print_cross_y_right_line2 = [
            (i + 1, point[0], point[1]) for i, point in enumerate(self.print_cross_y_right_line2)
        ]
        self.print_cross_y_left_line3 = [
            (i + 1, point[0], point[1]) for i, point in enumerate(self.print_cross_y_left_line3)
        ]
        self.print_cross_y_right_line3 = [
            (i + 1, point[0], point[1]) for i, point in enumerate(self.print_cross_y_right_line3)
        ]
        ...
        # 管孔数量为4个
            ...
            x_up_count_line2 = sum(1 for coord in current_coords if coord in self.original_print_cross_x_up_line2)
            x_down_count_line2 = sum(1 for coord in current_coords if coord in self.original_print_cross_x_down_line2)

            y_left_count_line2 = sum(1 for coord in current_coords if coord in self.original_print_cross_y_left_line2)
            y_right_count_line2 = sum(1 for coord in current_coords if coord in self.original_print_cross_y_right_line2)

            x_up_count_line3 = sum(1 for coord in current_coords if coord in self.original_print_cross_x_up_line3)
            x_down_count_line3 = sum(1 for coord in current_coords if coord in self.original_print_cross_x_down_line3)

            y_left_count_line3 = sum(1 for coord in current_coords if coord in self.original_print_cross_y_left_line3)
            y_right_count_line3 = sum(1 for coord in current_coords if coord in self.original_print_cross_y_right_line3)
            # x轴第一排
            ...
            # y轴第一排
                ...
                self.cross_y_4_pipes(current_coords, self.print_cross_y_left_line1, self.print_cross_y_right_line1)
                self.is_y_line1 = True
            # x轴第二排
            elif x_up_count_line2 == 2 and x_down_count_line2 == 2:
                if self.is_x_line1:
                    self.cross_x_4_pipes(current_coords, self.print_cross_x_up_line2, self.print_cross_x_down_line2)
                    self.is_x_line2 = True
                else:
                    QMessageBox.warning(self, "选择错误", "请从第1排（行）依次完成交叉布管")
                    self.clear_selection_highlight()
            ...
            # y轴第二排
                ...
                if self.is_y_line1:
                    self.cross_y_4_pipes(current_coords, self.print_cross_y_left_line2, self.print_cross_y_right_line2)
                    self.is_y_line2 = True
                else:
                    QMessageBox.warning(self, "选择错误", "请从第1排（行）依次完成交叉布管")
                    self.clear_selection_highlight()
            ...
            # x轴第三排
                ...
                if self.is_x_line1 and self.is_x_line2:
                    self.cross_x_4_pipes(current_coords, self.print_cross_x_up_line3, self.print_cross_x_down_line3)
                    self.is_x_line3 = True
                else:
                    QMessageBox.warning(self, "选择错误", "请从第1排（行）依次完成交叉布管")
                    self.clear_selection_highlight()
            ...
            # y轴第三排
                ...
                if self.is_y_line1 and self.is_y_line2:
                    self.cross_y_4_pipes(current_coords, self.print_cross_y_left_line3, self.print_cross_y_right_line3)
                    self.is_y_line3 = True
                else:
                    QMessageBox.warning(self, "选择错误", "请从第1排（行）依次完成交叉布管")
                    self.clear_selection_highlight()
