...
def edit_slide(self, slide_item):
    ...
    def _apply_dialog_visibility():
        ...
        # 滑道数量：仅板式显示
        try:
            c = row_containers.get("滑道数量")
            if c is not None:
                c.setVisible(bool(is_plate))
        except Exception:
            pass
        # 滑道方位角：板式且数量=1
        try:
            c = row_containers.get("滑道方位角")
            if c is not None:
                c.setVisible(show_azimuth)
        except Exception:
            pass
        # 高/厚/切边：圆钢条式隐藏
        for k in [
        ...
        # 夹角：板式且数量≠1 时显示
        try:
            c = row_containers.get("滑道与竖直中心线夹角")
            if c is not None:
                c.setVisible((not bool(is_round)) and (not show_azimuth))
        except Exception:
            pass
        # 导轨类型：仅 AKU/BKU 显示
        try:
    ...
    try:
        ...
        input_widgets["滑道数量"].currentTextChanged.connect(lambda _t: _apply_dialog_visibility())
    except Exception:
        pass
