...
def insert_tube_row_counts_to_db(sorted_centers: List[List[Tuple[float, float]]], product_id: str):
    ...
    try:
        with connection.cursor() as cursor:
            # 删除指定产品ID对应的旧记录，避免误删全表
            delete_sql = "DELETE FROM 产品设计活动表_布管数量表 WHERE 产品ID = %s"
            cursor.execute(delete_sql, (product_id,))
            print(f"🗑️ 已清除产品ID={product_id} 的旧布管数量记录")
...
# ✅ 主流程
def process_and_save_to_quantity_table(json_path: str, product_id: str = "PD20250619011"):
...
if __name__ == "__main__":
    json_file_path = "dependencies/中间数据/布管输出参数.json"
