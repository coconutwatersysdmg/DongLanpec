...
def update_config_directory(ini_path):
    ...
    abs_dependency_path = os.path.join(base_dir, 'dependencies').replace('\\', '/')

    # 初始化 configparser 并读取 ini 文件（使用 gbk 编码）
...
def update_project_directory(file_path, key_name):
    # 当前 Python 脚本所在目录
    base_dir = os.path.dirname(os.path.abspath(__file__))
    new_path = os.path.join(base_dir, 'dependencies').replace('\\', '/')
...
if __name__ == "__main__":
    ...
    # 示例使用
    update_config_directory("dependencies/config/config.ini")

    # 示例用法
    update_project_directory("dependencies/config/projectManagementInfo.ini", "PROJECT_DEFAULT_DIR")
