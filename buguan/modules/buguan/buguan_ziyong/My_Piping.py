...
class TubeLayoutEditor(QMainWindow):
    ...
    def load_initial_data(self):
            ...
            try:
                ...
                if component_conn:
                    with component_conn.cursor() as cursor:
                        ...
                        self.setup_parameters(default_params)
                        # 隐藏指定参数
                        self.hide_specific_params(hidden_params)
                        self.update_leftpad_params()
            except Exception as e:
                print(f"默认参数加载错误: {str(e)}")  # 增加控制台输出便于调试
                QMessageBox.critical(self, "加载错误", f"无法读取默认参数: {str(e)}")
            finally:
                # 确保连接只在未关闭时关闭
                if component_conn and component_conn.open:
                    component_conn.close()

        self.calculate_piping_layout()
        self.calculate_piping_layout()
        side_dangban_thick = float(self.input_json['LB_BPBThick'])
        baffle_thickness = float(self.input_json['LB_BaffleThick'])
        baffle_angle = float(self.input_json['LB_BaffleA'])
        tube_outer_diameter = float(self.input_json['LB_TubeD'])
        tube_pitch = float(self.input_json['LB_S'])
        height = float(self.input_json['LB_SlipWayHeight'])
        thickness = float(self.input_json['LB_SlipWayThick'])
        angle = float(self.input_json['LB_SlipWayAngle'])
        ...
        try:
            product_conn = create_product_connection()
            if product_conn and product_conn.open:  # 确保连接已打开
            ...
            # 更详细地打印异常信息
            print(f"查询是否布置滑道错误: 类型={type(e)}, 内容={e}")
        finally:
            if product_conn and product_conn.open:
                product_conn.close()

        lagan_centers = all_coords['lagan_centers']
        side_centers = all_coords["side_centers"]
        center_dangguan_centers = all_coords["center_dangguan_centers"]
        side_dangban_centers = all_coords["side_dangban_centers"]
        center_dangban_centers = all_coords["center_dangban_centers"]
        impingement_plate_1_centers = all_coords["impingement_plate_1_centers"]
        impingement_plate_2_centers = all_coords["impingement_plate_2_centers"]
        del_centers = all_coords["del_centers"]
        ...
        for i in range(0, len(centers_list), 2):
            if i + 1 < len(centers_list):
                pair = [centers_list[i], centers_list[i + 1]]
                self.build_impingement_plate(pair, "与定距管/拉杆焊接平板式",
        ...
        for i in range(0, len(centers_list), 2):
            if i + 1 < len(centers_list):
                pair = [centers_list[i], centers_list[i + 1]]
                self.build_impingement_plate(pair, "与定距管/拉杆焊接折边式",
                                             baffle_thickness, baffle_angle,
                                             0, 0, 0, tube_outer_diameter, tube_pitch
                                             )
    ...
    def update_SN(self):
        """根据管程数的值更新分程隔板两侧相邻管中心距Sn（竖直）所在行的状态"""
        # 1. 查找管程数和分程隔板两侧相邻管中心距Sn（竖直）在参数表中的行索引
        tube_pass_row = -1
        sn_row = -1
        row_count = self.param_table.rowCount()
        ...
        for row in range(row_count):
            ...
            if param_name == "管程程数":
                tube_pass_row = row
            elif param_name == "分程隔板两侧相邻管中心距Sn（竖直）":
                sn_row = row
        ...
        # 4. 更新分程隔板两侧相邻管中心距Sn（竖直）所在整行的状态
        if sn_row != -1:
            # 设置整行的每个单元格
            for col in range(self.param_table.columnCount()):
                # 获取单元格控件或项目
                cell_widget = self.param_table.cellWidget(sn_row, col)
                cell_item = self.param_table.item(sn_row, col) if not cell_widget else None

                if tube_pass == 2:
                    # 管程数为2时，设置为灰色不可编辑
                    if cell_widget:
                        cell_widget.setEnabled(False)
                        cell_widget.setStyleSheet("background-color: #f0f0f0;")  # 灰色背景
                    if cell_item:
                        # 取消编辑权限
                    ...
                    if cell_widget:
                        ...
                        cell_widget.setStyleSheet("")  # 恢复默认样式
                    if cell_item:
                        # 恢复编辑权限
                        cell_item.setFlags(cell_item.flags() | Qt.ItemIsEditable)
                        # 恢复默认背景色
    ...
    def build_impingement_plate(self, selected_centers, baffle_type, baffle_thickness, baffle_angle,
        ...
        if baffle_type == "与定距管/拉杆焊接平板式":
            ...
            line = QLineF(QPointF(points[0][0], points[0][1]), QPointF(points[1][0], points[1][1]))
            ...
            self.operations.append({
                ...
                "angle": baffle_angle,
                "points": points,
                "interfering_tubes": [center for center in
                                      self.operations[-1].get("interfering_tubes", [])] if self.operations else []
            })
            ...
            combined = []
            seen = set()
            for coord in self.impingement_plate_2:
                if coord not in seen:
                    seen.add(coord)
                    combined.append(coord)
            for coord in selected_centers_list:
                    ...
                    if row_idx < len(centers_group) and col_idx < len(centers_group[row_idx]):
                        ...
                        for item in self.graphics_scene.items(click_point):
                            if isinstance(item, QGraphicsEllipseItem):
                                self.graphics_scene.removeItem(item)
                                break
                self.selected_centers.clear()  # 清空选中记录
                return  # 选中数量错误时终止
            # 提取两个选中圆心的坐标并清除选中标记
            points = []
            for row_label, col_label in selected_centers:
            ...
            if len(points) != 2:
                ...
                return  # 坐标获取失败时终止
            # 计算折边式防冲板的关键坐标点
            A = QPointF(points[0][0], points[0][1])  # 起点（第一个圆心）
            B = QPointF(points[1][0], points[1][1])  # 终点（第二个圆心）
            AB_vector = B - A  # 向量AB
            AB_length = math.hypot(AB_vector.x(), AB_vector.y())  # 计算AB长度
            # 处理两点重合的异常情况
            if AB_length == 0:
                QMessageBox.warning(self, "错误", "两个选中的圆心位置重合，无法绘制防冲板")
                return  # 两点重合时终止
            # 计算坐标轴向量（用于折边角度计算）
            x_axis = AB_vector / AB_length  # 单位化AB向量作为x轴
            y_axis = self.determine_y_axis(A, B, x_axis)  # 获取垂直于AB的y轴向量（假设该方法已实现）
            # 计算防冲板几何参数
            angle_rad = math.radians(baffle_angle)  # 角度转弧度
            # 基于换热管参数计算防冲板高度（取两个计算值的最大值）
            ...
            # 计算折边倾斜段长度和顶部水平段长度
            incline_length = baffle_height / math.sin(angle_rad)  # 倾斜边长度
            top_length = AB_length - 2 * (baffle_height / math.tan(angle_rad))  # 顶部水平段长度
            # 验证顶部长度是否有效（不能为负）
            if top_length < 0:
                QMessageBox.warning(
                    self, "参数异常",
            ...
            Q = P + x_axis * top_length  # 顶部终点
            # 绘制折边式防冲板（三段折线）
            pen = QPen(QColor(0, 0, 139))  # 深蓝色线条
            pen.setWidth(int(baffle_thickness)) if baffle_thickness else pen.setWidth(3)  # 设置线宽（默认3）
            self.graphics_scene.addLine(QLineF(A, P), pen)  # 绘制左侧倾斜边
            self.graphics_scene.addLine(QLineF(P, Q), pen)  # 绘制顶部水平边
            self.graphics_scene.addLine(QLineF(Q, B), pen)  # 绘制右侧倾斜边
