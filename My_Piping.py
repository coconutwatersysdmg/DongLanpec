...
class TubeLayoutEditor(QMainWindow):
        ...
        for row, param in enumerate(params):
            ...
            if param['参数名'] in ["是否以外径为基准", "分程布置形式", "换热管排列方式", "滑道定位",
                ...
                if param['参数名'] == "是否以外径为基准":
                    ...
                    combo.setIconSize(QSize(75, 55))   # 或者 (80, 60)，随你调大小
    ...
    def find_closest_to_axes(self):
        if self.sorted_current_centers_up and self.sorted_current_centers_down:
            ...
            self.print_cross_x_up_line1 = self.sorted_current_centers_up[0] if self.sorted_current_centers_up else []
            self.print_cross_x_down_line1 = self.sorted_current_centers_down[0] if self.sorted_current_centers_down else []
            ...
            for row in self.sorted_current_centers_up:
                for x, y in row:
                    ...
                    if abs(x) == min_x_up:
                        if x < 0:
                            self.print_cross_y_left_line1.append((x, y))
                        else:
                            self.print_cross_y_right_line1.append((x, y))
            ...
            if not self.print_cross_y_left_line1 and not self.print_cross_y_right_line1:
                all_points = []
                ...
                if all_points:
                    ...
                    if closest[0] < 0:
                        self.print_cross_y_left_line1 = [closest]
                    else:
                        self.print_cross_y_right_line1 = [closest]

                    symmetric = (-closest[0], closest[1])
                    if symmetric in all_points:
                        if symmetric[0] < 0:
                            self.print_cross_y_left_line1.append(symmetric)
                        else:
                            self.print_cross_y_right_line1.append(symmetric)

            self.print_cross_y_left_line1.sort(key=lambda p: abs(p[1]))
            self.print_cross_y_right_line1.sort(key=lambda p: abs(p[1]))
        else:
    ...
    def get_selected_x_center_numbers(self, selected_centers):
        ...
        if up_number is None or down_number is None:
            raise ValueError("selected_centers中的坐标未完全匹配到self.print_cross_x_up_line1或self.print_cross_x_down_line1")
    ...
    def get_selected_y_center_numbers(self, selected_centers):
        ...
        if left_number is None or right_number is None:
            raise ValueError("selected_centers中的坐标未完全匹配到self.print_cross_y_left_line1或self.print_cross_y_right_line1")
    ...
    def get_x_2_number_sequences(self, result):
        ...
        # 验证过滤后长度仍相等
        assert len(self.pair_x_info_up_line1) == len(self.pair_x_info_down_line1), "过滤后序列长度必须相等"
